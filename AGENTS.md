...
- Keep collective-specific state under canonical repo-local surfaces such as `private/universal/identity/`, `private/universal/vaults/openclaw/state/`, and `memory/.docc/`.
- Treat `workspace` as the primary working home.
- Keep the commissioned identity in `private/universal/identity/`.
- Keep canonical memory in `memory/.docc/`.
