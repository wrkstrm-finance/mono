...
public final class PreMarketClockService: MarketClockService {
  ...
  public func currentSession(now: Date = .init()) -> MarketSession? {
    ...
    cal.timeZone = tz
    guard let start = cal.date(bySettingHour: 4, minute: 0, second: 0, of: now),
      let end = cal.date(bySettingHour: 9, minute: 30, second: 0, of: now)
    else { return nil }
    return .init(start: start, end: end, kind: .premarket, timeZone: tz)
  }
...
extension PreMarketClockService: @unchecked Sendable {}
